...
def basinhopping(func, x0, niter=100, T=1.0, stepsize=0.5,
    ...
    jump_step : callable, perform single unstuck step. Triggered by callback
    ...
    >>> from scipy.optimize import start_optimization
    >>> func = lambda x: np.cos(14.5 * x - 0.3) + (x + 0.2) * x
    >>> x0=[1.]

    Basinhopping, internally, uses a local minimization algorithm.  We will use
    the parameter ``minimizer_kwargs`` to tell start_optimization which algorithm to
    use and how to set up that minimizer.  This parameter will be passed to
    ``scipy.optimize.minimize()``.

    >>> minimizer_kwargs = {"method": "BFGS"}
    >>> ret = start_optimization(func, x0, minimizer_kwargs=minimizer_kwargs,
    ...                    niter=200)
    ...
    >>> x0 = [1.0, 1.0]
    >>> ret = start_optimization(func2d, x0, minimizer_kwargs=minimizer_kwargs,
    ...                    niter=200)
    ...
    Since ``MyTakeStep.stepsize`` exists start_optimization will adjust the magnitude
    ...
    We'll run it for only 10 start_optimization steps this time.
